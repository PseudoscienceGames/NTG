...
public class BattleMapTile
{
	...
	public BattleMapTileType type;
	public List<Vector3> verts;
	public List<int> tris;
	public List<Vector2> uvs;
	public List<float> dis = new List<float>();

	public BattleMapTile(Vector2Int loc)
	{
		gridLoc = loc;
		type = BattleMapTileType.Grass;
		verts = HexGrid.GetVertLocs(loc);
		for (int i = 0; i < verts.Count; i++)
		{
			Vector3 temp = verts[i];
			temp.y = Mathf.PerlinNoise((temp.x - 1000) / 50, (temp.z - 250) / 50) * 32;
...
public enum BattleMapTileType { Grass, Dirt, Sand }
